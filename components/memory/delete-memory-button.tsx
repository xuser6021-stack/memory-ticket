...
export default function DeleteMemoryButton({ memoryId }: DeleteMemoryButtonProps) {
  ...
  return (
    <AlertDialog.Root>
      <AlertDialog.Trigger className="inline-flex rounded-lg bg-destructive px-4 py-2 text-sm font-medium text-white hover:bg-destructive/90">
        Delete Memory
      </AlertDialog.Trigger>
      <AlertDialog.Portal>
        <AlertDialog.Backdrop className="fixed inset-0 z-50 bg-black/50" />
        <AlertDialog.Popup className="fixed left-1/2 top-1/2 z-50 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 space-y-5 rounded-xl border bg-card p-6 text-card-foreground shadow-lg">
